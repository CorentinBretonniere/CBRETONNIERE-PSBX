...
La fonction R "D" permet de dériver des _expressions_ définies au préalable
```{r}
...
D'abord, il faut assigner à notre variable une fonction (maDerive <- function(x)), cette fonction est l'expression de la dérivée (monExpression <- deriv(~2*x^4 + sqrt(x), "x")).
...
On peut également appliquer cette dérivé à toutes les valeurs d'un vecteur ...
Pour ce faire il faut lui appliquer la fonction "Vectorize". 
